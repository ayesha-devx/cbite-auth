          ...
          {steps.map((step, idx) => (
            <div key={idx} className="relative group text-left">
              ...
              <div className="flex items-center justify-between mb-6">
                <span className="text-5xl font-black text-slate-200 select-none group-hover:text-brand-blue-100 transition-colors">
                  {step.num}
                </span>
                <div className="w-10 h-10 rounded-lg bg-white border border-slate-200/80 shadow-xs flex items-center justify-center">
                  {step.icon}
                </div>
              </div>

              {/* Step Title */}
              <h3 className="text-lg font-bold text-brand-blue-950 mb-2 group-hover:text-brand-blue-500 transition-colors">
                {step.title}
              </h3>
              ...
              {/* Linking connector line for desktop */}
              {idx < 3 && (
                <div className="hidden lg:block absolute top-6 -right-4 w-8 h-px bg-slate-200 pointer-events-none"></div>
              )}
            </div>
          ))}
        </div>
